...
observe :: (Foldable f1, Foldable f2, Foldable f3, Ord x)
  => [(String, x -> Bool)]
  ...
  -> f1 (f2 (schema1, (Maybe (Ann s1 m x), Ann s1 m x)))
  -- ^ First set of schemas. Observations are made between schemas of the two sets.
  -> f3 (schema2, (Maybe (Ann s2 m x), Ann s2 m x))
  -- ^ Second set of schemas.
  -> (ChurchList (schema2, (Maybe (Ann s2 m x), Ann s2 m x)), ChurchList Observation)
...
allObservations :: Ord x
  => [(String, x -> Bool)]
  -> (TypeRep -> Char)
  -> (Maybe (Ann s1 m x), Ann s1 m x)
  -> (Maybe (Ann s2 m x), Ann s2 m x)
